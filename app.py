...
path=r"C:\SUFIYAN\STUDY MATERIALS\FOR CLASS\FOR CLASS\sagemaker-flights-price-prediction\data\train.csv"
...
st.markdown(
    ...
    <h1 style='text-align: center; font-size: 40px;'>
        ...
        <span style='color: orange;'>SageMaker</span>
    </h1>
    """, 
    unsafe_allow_html=True
)

st.markdown("<h3 style='color: black;'>Enter Flight Details:</h3>", unsafe_allow_html=True)
...
xg=r'C:\SUFIYAN\STUDY MATERIALS\FOR CLASS\FOR CLASS\sagemaker-flights-price-prediction\xgboost-model'

if st.button("Predict"):
    saved_preprocessor = joblib.load("preprocessor.joblib")
    x_new_pre = saved_preprocessor.transform(x_new)

    with open(xg, "rb") as f:
        model = pickle.load(f)
    x_new_xgb = xgb.DMatrix(x_new_pre)
    pred = model.predict(x_new_xgb)[0]

    st.markdown(f"<h2 style='color: white;'>The predicted price is <b><u>{pred:.0f} </b></u>INR</h2>", unsafe_allow_html=True)
